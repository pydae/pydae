...
lead-lag turbine representation.  The model matches the PSS/E TGOV1 definition
and the IEEE recommended practice.
...
The anti-windup term $K_{awu}$ drives $x_1$ back when the position limiter
saturates.

The turbine is represented by a lead-lag $(1 + T_2\,s)/(1 + T_3\,s)$ with
state $x_2$:
...
```{eval-rst}
.. automodule:: pydae.bps.govs.tgov1
   :no-members:
```
