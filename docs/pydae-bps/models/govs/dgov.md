...
The `dgov` model is instantiated by including an entry in the relevant
...
| $K$ | `K_gov` | 1.0 | pu | Governor proportional gain |
| $T_1$ | `T_1_gov` | 0.02 | s | Governor lag time constant |
| $T_2$ | `T_2_gov` | 0.3 | s | Fuel valve actuator time constant |
| $R_{open}$ | `R_open_gov` | 0.3 | pu/s | Maximum valve opening rate |
| $R_{close}$ | `R_close_gov` | -0.3 | pu/s | Maximum valve closing rate (negative) |
| $V_{max}$ | `V_max_gov` | 1.0 | pu | Maximum fuel valve position |
| $V_{min}$ | `V_min_gov` | 0.0 | pu | Minimum fuel valve position |
| $T_3$ | `T_3_gov` | 0.5 | s | Engine combustion lag time constant |
| $K_{turb}$ | `K_turb_gov` | 1.0 | pu | Engine/turbine gain |
| $W_{fnl}$ | `W_fnl_gov` | 0.0 | pu | No-load fuel flow |
...
| Symbol | Variable | Default | Units | Description |
|---|---|---|---|---|
| $p_c$ | `p_c` | 0.8 | pu | Fuel valve setpoint (load reference). At steady state: p_m = K_turb*(p_c - W_fnl). |
