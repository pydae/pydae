...
out[0] = (p[3]*sin(p[2]*x[1]) + u[0] - x[0])/p[1];
out[1] = -x[1]*(1 - u[1]) + u[1];
...
out[0] = p[0]*(p[3]*sin(p[2]*x[1]) + u[0] - x[0])/p[1] + x[0];
out[1] = p[3]*sin(p[2]*x[1]) + u[0];
out[2] = x[1];

}
void de_jac_ini_xy_eval(double *out,double *x,double *y,double *u,double *p,double Dt){

out[1] = p[3]*p[2]*cos(p[2]*x[1])/p[1];
...
#ifdef PYPY_VERSION
PyMODINIT_FUNC
_cffi_pypyinit_ctrl_leadlag_cffi(const void *p[])
{
...
#  ifdef _MSC_VER
     PyMODINIT_FUNC
#  if PY_MAJOR_VERSION >= 3
     PyInit_ctrl_leadlag_cffi(void) { return NULL; }
#  else
     initctrl_leadlag_cffi(void) { }
#  endif
#  endif
#elif PY_MAJOR_VERSION >= 3
PyMODINIT_FUNC
PyInit_ctrl_leadlag_cffi(void)
{
  return _cffi_init("ctrl_leadlag_cffi", 0x2601, &_cffi_type_context);
}
#else
PyMODINIT_FUNC
initctrl_leadlag_cffi(void)
{
  _cffi_init("ctrl_leadlag_cffi", 0x2601, &_cffi_type_context);
}
#endif
