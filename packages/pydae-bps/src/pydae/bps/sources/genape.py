...
def genape(grid, name, bus_name, data_dict):
    ...
    # inputs
    V_sm = backend.symbols(f"V_{bus_name}")
    ...
    T_v = backend.symbols(f"T_v_{name}")
    v_ref_filtered = backend.symbols(f"v_ref_filtered_{name}")
    ...
    V_sr = V_sm * cos(theta_s)
    V_si = V_sm * sin(theta_s)

    V_vm = v_ref_filtered + Dv
    V_vr = V_vm * cos(theta_v)
    V_vi = V_vm * sin(theta_v)

    denom = R_v ** 2 + X_v ** 2
    p_pu = (R_v * V_si ** 2 - R_v * V_si * V_vi + R_v * V_sr ** 2
            - R_v * V_sr * V_vr + V_si * V_vr * X_v - V_sr * V_vi * X_v) / denom
    q_pu = (-R_v * V_si * V_vr + R_v * V_sr * V_vi + V_si ** 2 * X_v
            - V_si * V_vi * X_v + V_sr ** 2 * X_v - V_sr * V_vr * X_v) / denom

    # dynamic equations
    ddelta = Omega_b * (omega - omega_s) - K_delta * (delta - delta_ref)
    dDomega = alpha - K_alpha * Domega
    dDv = rocov - K_rocov * Dv
    dv_ref_filtered = 1 / T_v * (v_ref - v_ref_filtered)

    # algebraic equations
    g_omega = -omega + Domega + omega_ref

    # dae
    f_vsg = [ddelta, dDomega, dDv, dv_ref_filtered]
    x_vsg = [delta, Domega, Dv, v_ref_filtered]
    g_vsg = [g_omega]
    y_vsg = [omega]
    ...
    grid.dae['xy_0_dict'].update({str(v_ref_filtered): 1.0})
    ...
    grid.dae['params_dict'].update({f"T_v_{name}": 0.02})
