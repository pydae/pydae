...
def ntsieeeg1(dae, syn_data, name, bus_name, backend=None):
    '''
 ...
 "K_2":0,"K_4":0,"K_6":0,"K_8":0,"U_0":0.5, "U_c":-0.5,"p_c":0.1}
    ...
    T_7  = backend.symbols(f"T_7_gov_{name}")
    ...
    U_0  = backend.symbols(f"U_0_gov_{name}")
    ...
    y_g_nosat = x_3
    y_g = backend.Piecewise((P_max, y_g_nosat > P_max), (P_min, y_g_nosat < P_min), (y_g_nosat, True))

    u_3 = K * Domega + p_c - y_g + K_sec * p_agc
    u_g = backend.Piecewise((U_c, u_3 / T_3 < U_c), (U_0, u_3 / T_3 > U_0), (u_3 / T_3, True))
