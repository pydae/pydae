...
if dae_file_mode == 'local':
    import temp_m2_cffi as jacs
if dae_file_mode == 'enviroment':
    import envus.no_enviroment.temp_m2_cffi as jacs
if dae_file_mode == 'colab':
    import temp_m2_cffi as jacs
if dae_file_mode == 'testing':
    from pydae.temp import temp_m2_cffi as jacs
...
class model: 
    ...
    def __init__(self,matrices_folder='./build'): 
        ...
        self.N_z = 4 
        self.N_store = 100000 
        self.params_list = ['S_base', 'g_1_2', 'b_1_2', 'bs_1_2', 'g_shunt_1', 'b_shunt_1', 'U_1_n', 'U_2_n', 'Omega_b_1', 'S_n_1', 'H_1', 'X1d_1', 'X1q_1', 'D_1', 'R_a_1', 'K_delta_1', 'K_sec_1', 'K_p_agc', 'K_i_agc', 'K_xif'] 
        self.params_values_list  = [100000000, 7.692307692307692, -38.46153846153846, 2e-06, 0.0, -1e-06, 20000, 20000, 314.1592653589793, 200000000, 5, 0.3, 0.55, 0, 0.01, 0, 0, 0, 0, 0.01] 
        self.inputs_ini_list = ['P_1', 'Q_1', 'P_2', 'Q_2', 'e1q_1', 'p_m_1', 'v_ref_2', 'theta_ref_2'] 
        self.inputs_ini_values_list  = [0, 0, 0, 0, 1.0, 0.5, 1.0, 0.0] 
        self.inputs_run_list = ['P_1', 'Q_1', 'P_2', 'Q_2', 'e1q_1', 'p_m_1', 'v_ref_2', 'theta_ref_2'] 
        self.inputs_run_values_list = [0, 0, 0, 0, 1.0, 0.5, 1.0, 0.0] 
        self.outputs_list = ['V_1', 'V_2', 'p_e_1', 'V_dummy_2'] 
        self.x_list = ['delta_1', 'omega_1', 'V_dummy_2', 'xi_freq'] 
        ...
        if self.dae_file_mode == 'enviroment':
            fobj = BytesIO(pkgutil.get_data(__name__, f'./temp_m2_sp_jac_ini_num.npz'))
            self.sp_jac_ini = sspa.load_npz(fobj)
        else:
            self.sp_jac_ini = sspa.load_npz(f'{self.matrices_folder}/temp_m2_sp_jac_ini_num.npz')
        ...
        if self.dae_file_mode == 'enviroment':
            fobj = BytesIO(pkgutil.get_data(__name__, './temp_m2_sp_jac_run_num.npz'))
            self.sp_jac_run = sspa.load_npz(fobj)
        else:
            self.sp_jac_run = sspa.load_npz(f'{self.matrices_folder}/temp_m2_sp_jac_run_num.npz')
        ...
        if self.dae_file_mode == 'enviroment':
            fobj = BytesIO(pkgutil.get_data(__name__, './temp_m2_sp_jac_trap_num.npz'))
            self.sp_jac_trap = sspa.load_npz(fobj)
        else:
            self.sp_jac_trap = sspa.load_npz(f'{self.matrices_folder}/temp_m2_sp_jac_trap_num.npz')
        ...
        self.sp_Fu_run = sspa.load_npz(f'{self.matrices_folder}/temp_m2_Fu_run_num.npz')
        self.sp_Gu_run = sspa.load_npz(f'{self.matrices_folder}/temp_m2_Gu_run_num.npz')
        self.sp_Hx_run = sspa.load_npz(f'{self.matrices_folder}/temp_m2_Hx_run_num.npz')
        self.sp_Hy_run = sspa.load_npz(f'{self.matrices_folder}/temp_m2_Hy_run_num.npz')
        self.sp_Hu_run = sspa.load_npz(f'{self.matrices_folder}/temp_m2_Hu_run_num.npz')        
...
def sp_jac_ini_vectors():
    ...
    sp_jac_ini_ia = [0, 1, 12, 0, 1, 4, 5, 8, 9, 12, 2, 3, 12, 4, 5, 6, 7, 10, 4, 5, 6, 7, 11, 6, 7, 0, 4, 5, 8, 9, 0, 4, 5, 8, 9, 0, 4, 5, 8, 9, 10, 0, 4, 5, 8, 9, 11, 1, 12, 3, 12, 13]
    sp_jac_ini_ja = [0, 3, 10, 11, 13, 18, 23, 24, 25, 30, 35, 41, 47, 49, 52]
    sp_jac_ini_nia = 14
    sp_jac_ini_nja = 14
...
def sp_jac_run_vectors():
    ...
    sp_jac_run_ia = [0, 1, 12, 0, 1, 4, 5, 8, 9, 12, 2, 3, 12, 4, 5, 6, 7, 10, 4, 5, 6, 7, 11, 6, 7, 0, 4, 5, 8, 9, 0, 4, 5, 8, 9, 0, 4, 5, 8, 9, 10, 0, 4, 5, 8, 9, 11, 1, 12, 3, 12, 13]
    sp_jac_run_ja = [0, 3, 10, 11, 13, 18, 23, 24, 25, 30, 35, 41, 47, 49, 52]
    sp_jac_run_nia = 14
    sp_jac_run_nja = 14
...
def sp_jac_trap_vectors():
    ...
    sp_jac_trap_ia = [0, 1, 12, 0, 1, 4, 5, 8, 9, 12, 2, 3, 12, 4, 5, 6, 7, 10, 4, 5, 6, 7, 11, 6, 7, 0, 4, 5, 8, 9, 0, 4, 5, 8, 9, 0, 4, 5, 8, 9, 10, 0, 4, 5, 8, 9, 11, 1, 12, 3, 12, 13]
    sp_jac_trap_ja = [0, 3, 10, 11, 13, 18, 23, 24, 25, 30, 35, 41, 47, 49, 52]
    sp_jac_trap_nia = 14
    sp_jac_trap_nja = 14
