...
def test_milano_steady_state(model_type):
    ...
    try:
        # 2. Build and Compile the CFFI Model
        ...
        if model_type == "milano2ord":
            ...
            inputs = {'p_m_1': 0.5, 'e1q_1': 1.05} 
        else:
            # 3rd, 4th, 6th order use field voltage as input
            inputs = {'p_m_1': 0.5, 'v_f_1': 1.05}
        ...
        assert p_g == pytest.approx(p_e, abs=1e-3), f"{model_type} failed: p_g_1 doesn't match p_e_1"
