...
if dae_file_mode == 'local':
    import pv_cffi as jacs
if dae_file_mode == 'enviroment':
    import envus.no_enviroment.pv_cffi as jacs
...
class model: 
    ...
    def __init__(self): 
        ...
        self.N_z = 1 
        self.N_store = 100000 
        self.params_list = ['I_sc', 'I_mpp', 'V_mpp', 'V_oc', 'N_s', 'K_vt', 'K_it', 'R_s', 'R_sh', 'K_d'] 
        self.params_values_list  = [3.87, 42.1, 3.56, 33.7, 72, -0.16, 0.065, 0.5602, 1862, 1.3433] 
        self.inputs_ini_list = ['v', 'irrad', 'temp_deg', 'u_dummy'] 
        self.inputs_ini_values_list  = [30, 1000, 25, 1.0] 
        self.inputs_run_list = ['v', 'irrad', 'temp_deg', 'u_dummy'] 
        self.inputs_run_values_list = [30, 1000, 25, 1.0] 
        self.outputs_list = ['I_ph'] 
        self.x_list = ['x_dummy'] 
        self.y_run_list = ['i', 'p', 'v_mpp'] 
        self.xy_list = self.x_list + self.y_run_list 
        self.y_ini_list = ['i', 'p', 'v_mpp'] 
        ...
        if self.dae_file_mode == 'enviroment':
            fobj = BytesIO(pkgutil.get_data(__name__, f'./pv_sp_jac_ini_num.npz'))
            self.sp_jac_ini = sspa.load_npz(fobj)
        else:
            self.sp_jac_ini = sspa.load_npz(f'./{self.matrices_folder}/pv_sp_jac_ini_num.npz')
        ...
        if self.dae_file_mode == 'enviroment':
            fobj = BytesIO(pkgutil.get_data(__name__, './pv_sp_jac_run_num.npz'))
            self.sp_jac_run = sspa.load_npz(fobj)
        else:
            self.sp_jac_run = sspa.load_npz(f'./{self.matrices_folder}/pv_sp_jac_run_num.npz')
        ...
        if self.dae_file_mode == 'enviroment':
            fobj = BytesIO(pkgutil.get_data(__name__, './pv_sp_jac_trap_num.npz'))
            self.sp_jac_trap = sspa.load_npz(fobj)
        else:
            self.sp_jac_trap = sspa.load_npz(f'./{self.matrices_folder}/pv_sp_jac_trap_num.npz')
        ...
        #self.sp_Fu_run = sspa.load_npz(f'./{self.matrices_folder}/pv_Fu_run_num.npz')
        #self.sp_Gu_run = sspa.load_npz(f'./{self.matrices_folder}/pv_Gu_run_num.npz')
        #self.sp_Hx_run = sspa.load_npz(f'./{self.matrices_folder}/pv_Hx_run_num.npz')
        #self.sp_Hy_run = sspa.load_npz(f'./{self.matrices_folder}/pv_Hy_run_num.npz')
        #self.sp_Hu_run = sspa.load_npz(f'./{self.matrices_folder}/pv_Hu_run_num.npz')        
