...
def descriptions():
    ...
    descriptions_list += [
        {"type": "Dynamic State", "tex": "x_{pq}", "data": "",
         "model": "x_pq", "default": "",
         "description": "State-space filter states (when A_pq/A_p provided)",
         "units": "-"}]
...
def pv_pq_ss(grid, name, bus_name, data_dict):
    r"""
    PV inverter with VRT and optional state-space p/q reference filter.

    The model is a grid-following inverter in stationary (r-i) coordinates
    whose inner current loops are ideal (algebraic).  Power references from
    the PPC can optionally pass through a state-space low-pass filter before
    driving the current references.  An LVRT trigger ramps the operating mode
    from PQ to arbitrary current injection.  Current magnitude is limited via a
    smooth saturation.
    ...
    bus_name : str
        Bus the inverter is connected to.
    data_dict : dict
        User-supplied parameters.  Optionally contains ``A_pq``/``B_pq``/
        ``C_pq``/``D_pq`` (MIMO filter) *or* ``A_p``/``B_p``/``C_p``/``D_p``
        (SISO P filter, optionally ``A_q``/``B_q``/``C_q``/``D_q`` for Q).
    ...
    # 4. Dynamic state – LVRT ramp
    lvrt_ext_ramp = backend.symbols(f"lvrt_ext_ramp_{name}")
    dlvrt_ext_ramp = (lvrt_ext - lvrt_ext_ramp) / backend.symbols(f"T_lvrt_{name}")
    ...
    # 7. Optional state-space filter for p/q references
    ...
    if "A_pq" in data_dict:
        ...
        sys_pq["dx"] = _replace(sys_pq["dx"], sys_pq["u"][0, 0], p_s_ppc)
        sys_pq["dx"] = _replace(sys_pq["dx"], sys_pq["u"][1, 0], q_s_ppc)
        sys_pq["z_evaluated"] = _replace(sys_pq["z_evaluated"], sys_pq["u"][0, 0], p_s_ppc)
        sys_pq["z_evaluated"] = _replace(sys_pq["z_evaluated"], sys_pq["u"][1, 0], q_s_ppc)
        ...
        sys_p["dx"] = _replace(sys_p["dx"], sys_p["u"][0, 0], p_s_ppc)
        sys_p["z_evaluated"] = _replace(sys_p["z_evaluated"], sys_p["u"][0, 0], p_s_ppc)
        ...
        sys_q["dx"] = _replace(sys_q["dx"], sys_q["u"][0, 0], q_s_ppc)
        sys_q["z_evaluated"] = _replace(sys_q["z_evaluated"], sys_q["u"][0, 0], q_s_ppc)
    ...
    # 9. PQ-mode current references (use filtered references)
    i_sr_pq = (p_s_ppc_d * v_sr + q_s_ppc_d * v_si) / v_sq_mag
    i_si_pq = (p_s_ppc_d * v_si - q_s_ppc_d * v_sr) / v_sq_mag

    # 10. Arbitrary-mode current references
    i_sr_ar = (i_sa_ref * v_sr + i_sr_ref * v_si) / v_m
    i_si_ar = (i_sa_ref * v_si - i_sr_ref * v_sr) / v_m

    # 11. Blend via LVRT ramp
    ...
    # 13. Injected power
    g_p_s = v_sr * i_sr + v_si * i_si - p_s
    g_q_s = v_si * i_sr - v_sr * i_si - q_s

    # 14. Assembly – VRT ramp dynamic state first, then algebraic
...
def test_build():
    ...
    grid = BpsBuilder("pv_pq_ss.hjson")
    grid.checker()
    grid.uz_jacs = False
    grid.construct("temp_pv_pq_ss")
...
def test_run():
    ...
    # Step in p reference
    model.run(5.0, {"p_s_ppc_1": 0.8})

    # LVRT trigger
    model.run(6.0, {"lvrt_ext_1": 1.0, "i_sa_ref_1": 0.5, "i_sr_ref_1": 0.3})
    model.run(8.0, {"lvrt_ext_1": 0.0})

    model.post()

    fig, axes = plt.subplots(3, 1, figsize=(8, 8))

    axes[0].plot(model.Time, model.get_values("p_s_1"), label="$p_s$")
    axes[0].set_ylabel("Active Power (pu)")
    axes[0].legend()
    axes[0].grid(True)

    axes[1].plot(model.Time, model.get_values("q_s_1"), label="$q_s$")
    axes[1].set_ylabel("Reactive Power (pu)")
    axes[1].legend()
    axes[1].grid(True)

    axes[2].plot(model.Time, model.get_values("lvrt_ext_ramp_1"),
                 label="$\\text{lvrt\\_ramp}$")
    axes[2].set_xlabel("Time (s)")
    axes[2].set_ylabel("LVRT ramp")
    axes[2].legend()
    axes[2].grid(True)

    fig.tight_layout()
    fig.savefig("pv_pq_ss_response.svg")
    print("Test completed. Plot saved as 'pv_pq_ss_response.svg'.")
