...
def pv_mpt_dcdc(grid,data,name,bus_name):
    '''
    ...
    v_l = v_dc
    epsilon_p = p_mp - p_l 
    p_mppt_h_ref = K_pp*epsilon_p + K_pi*xi_p + p_mp
    mppt = sym.Piecewise((0.0,p_r_ref<p_mppt_h_ref),(1.0,p_r_ref>=p_mppt_h_ref))  
    p_h = mppt*p_mppt_h_ref + (1.0-mppt)*p_r_ref
    v_h = v_hp - v_hn
    i_h = p_h/v_h
    i_hp =  i_h
    i_hn = -i_h
    ...
    grid.dae['u_ini_dict'].update({f'{str(p_r_ref)}':1e8})
    grid.dae['u_run_dict'].update({f'{str(p_r_ref)}':1e8})
...
def test():
    ...
    grid = UdsBuilder('pv_mpt_dcdc.hjson')
    grid.uz_jacs = True
    grid.construct('temp')
    grid.compile('temp')
    ...
    C = 0.02*S_n
    model = temp.model()
    model.ini({'A_D1':A,'B_D1':B,'C_D1':C,'p_r_ref_D1':500e3},'xy_0.json')
    model.report_u()
    model.report_y()
    model.report_z()
