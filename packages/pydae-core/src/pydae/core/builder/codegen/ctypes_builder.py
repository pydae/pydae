...
def generate_and_compile_ctypes(builder_obj):
    ...
    # Go up from codegen/ to builder/ to core/, then into solver/
    core_dir = os.path.dirname(os.path.dirname(codegen_dir))
    dae_dense_path = os.path.join(core_dir, 'solver', 'daesolver_dense.c')
